...
pub mod cosmicgate_sol {    
    ...
    pub fn update_task(ctx: Context<UpdateTask>, node_seed: Pubkey, task_seed: Pubkey, crc: String, status: u8, uuid: String) -> Result<()> {
        processors::update_task::exec(ctx, node_seed, task_seed, crc, status, uuid)
    }
